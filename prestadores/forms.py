...
from .models import PrestadorPessoaFisica, Especialidade, Fornecedor
...
class PrestadorPessoaFisicaForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Div(
                Div(Field('nm_prestador'), css_class="col-md-8", ),
                css_class="row",
            ),
            Div(
                ...
                css_class="row", ),
            Div(
                Div(Field('especialidades'), css_class="col-md-8", ),
                css_class="row", ),
            #Div(
            #    Div(Field('nm_pai'), css_class="col-md-6", ),
            #    Div(Field('ds_faculdade'), css_class="col-md-3", ),
                
            #    css_class="row", ),
            Div(
                Div(Field('ds_email'), css_class="col-md-4", ),
                css_class="row", ),
            Div(
    ...
    class Meta:
        ...
        #           'observacoes']
        fields = ['nm_prestador', 'nr_cpf_cgc', 'dt_nascimento', #'nm_mae', 'nm_pai','ds_codigo_conselho', 
                  'especialidades',
                  #'ds_faculdade', 
                  'ds_email', 'nr_cep', 'ds_endereco', 'ds_complemento', 'ds_bairro', 'nm_cidade',
                  'cd_uf', 
                  #'banco','ds_agencia', 'nr_conta', 
                  'nr_fone_contato', 
                  'identidade_frente', 
                  #'identidade_verso',
                  #'comprovante_residencia',
...
class FornecedorForm(ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        # It builds a default layout with all its fields
        self.helper = FormHelper(self)
        self.helper.form_id = 'id-forcedor-form'
        self.helper.form_method = 'post'
        # self.helper.form_action = reverse('index')
        self.helper.form_class = 'form-horizontal'
        self.helper.layout = Layout(
            Div(
                Div(Field('cd_fornecedor'), css_class="col-md-2", ),
                Div(Field('nm_fornecedor'), css_class="col-md-6", ),
                Div(Field('nm_fantasia'), css_class="col-md-6", ),
                css_class="row", ),
            Div(
                Div(Field('comprovante_cnpj'), css_class="col-md-4", ),
                css_class="row", ),
            Submit('submit', 'Enviar', css_class='btn-success col-centered')
        )
    class Meta:
        model = Fornecedor
        fields = ['cd_fornecedor','nm_fornecedor','nm_fantasia','comprovante_cnpj']